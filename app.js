...
// Auto Deployment
const deployment=require("./util/deployment.js")
...
const dao={
	...
	campaign:require("./dao/campaign.js")
};
...
// Deployment Webhook End Point
app.post("/api/"+cst.API_VERSION+"/deployment/update", function(req, res){
	deployment.update(function(error){
		if(error){
...
app.post("/api/"+cst.API_VERSION+"/admin/campaign", function(req, res){
	dao.campaign.insert(req).then(function(message){
...
app.get("/api/"+cst.API_VERSION+"/marketing/campaigns", function(req, res){
	let data=cache.get("campaigns"); // get from cache first
	if(data===null){ // get from database
		dao.campaign.list().then(function(results){
			// set cache for later use
			cache.set("campaigns", results);
